...
module Behaviors
  class RandomBlogger1k < Behaviors::RandomMover
    def initialize
      super
      @posts_added = 0
      @last_step = Time.new
    ...
    def next_action
      ...
      if @agent.status == 'running'
        ...
        #Madmass.logger.error "Agent #{@agent.id}, called #{(Time.new - @last_step).to_f} seconds after previous call"
      ...
      if not next_action
        #Madmass.logger.error "Agent #{@agent.id}, no action to perform, sleeping for 1s"
