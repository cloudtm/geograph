...
module CloudTm
  module RouteLoader
    ...
    def load_routes(gpx_path = nil)
      @hasRoutes = false
      tx_monitor do
        @hasRoutes = FenixFramework.getDomainRoot.getApp.hasAnyRoutes
      end
      unless @hasRoutes
        #Route.destroy_all
              ...
              if !rtes.blank?
                rtes.each do |rte|
                  ...
                  rte.css("rtept").each_with_index do |rtept, index|
                   # Madmass.logger.warn("loading position #{index}")
                  ...
                  Madmass.logger.warn "Created route #{route.name} with #{route.getPositionsCount} positions."
                end
              elsif !trks.blank?
                trks.each do |trk|
                  if trk.css("name").first
                    route_name = trk.css("name").first.inner_text.capitalize
                  else
                    route_name = "(unnamed #{rand(36**10).to_s(36)})"
                  end
                  route = retrieve_route(route_name)
                  c = 0
    ...
    #  Madmass.logger.warn "interpolating route of size #{route.size}"
    ...
    #
    #  end
    #  Madmass.logger.warn "new size is #{new_route.size}"
    #  Madmass.logger.warn "*****************************"
    #  return new_route
    #end
    #
