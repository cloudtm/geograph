...
function destroyGroup(group) {
  $.ajax({
    url: 'agent_groups/' + encodeURIComponent(group['id']),
    type: 'DELETE',
    dataType: 'script'
  });
}
...
function startGroup(group) {
  console.log(group);
  var url = 'agent_groups/' + encodeURIComponent(group['id']) + '/start';
  console.log(url);
  $.ajax({
    url: url,
    type: 'POST',
...
function stopGroup(group) {
  $.ajax({
    url: 'agent_groups/' + encodeURIComponent(group['id']) + '/stop',
    type: 'POST',
    dataType: 'script',
    headers: {
      'X-CSRF-Token': authenticity_token()
...
function pauseGroup(group) {
  $.ajax({
    url: 'agent_groups/' + encodeURIComponent(group['id']) + '/pause',
    type: 'POST',
    dataType: 'script',
    headers: {
      'X-CSRF-Token': authenticity_token()
