...
class BenchmarkService
  ...
  def run
    ...
    until @done
      ...
      current_group_knows_finished = false
      current_group_have_finished = true
      tx_monitor do
        CloudTm::AgentGroup.all.each do |agent_group|
          agent_group.agents.each do |agent|
            #Madmass.logger.error "##### Agent #{agent.inspect}, #{agent.class.name}, #{agent.respond_to?(:finished) ? agent.finished : 'n.a.'}"
            if agent.respond_to?(:finished)
              current_group_knows_finished = true
              if not agent.finished
                current_group_have_finished = false
              end
	      Madmass.logger.error "##### Agent #{agent.id} posts=#{agent.numOfPosts}, has finished? #{agent.finished ? 'YES' : 'no'}"
            end
          end
        end
      end

      time_is_over = (@current_static_profile.duration * @cycle_seconds) <= (Time.now - @step_start_time)
      Madmass.logger.error "#### Time is over: #{time_is_over}, agents knows finished: #{current_group_knows_finished}, agents have finished: #{current_group_have_finished}"
      if time_is_over and (not current_group_knows_finished or (current_group_knows_finished and current_group_have_finished))
