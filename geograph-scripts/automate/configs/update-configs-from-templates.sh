...
for i in 1 2 4 8 16 32 64 100; do
    ...
    find 001-2pc-gmu-repl3-${i}b/* | xargs sed -i "s/\$LOCAL_IP/$LOCAL_IP/"
    find 001-2pc-gmu-repl3-${i}b/* | xargs sed -i "s/\$DATABASE_PASSWORD/$DATABASE_PASSWORD/"
done
...
for pr in 2pc to pr switching; do
    ...
    find 002-$pr/* | xargs sed -i "s/\$LOCAL_IP/$LOCAL_IP/"
    find 002-$pr/* | xargs sed -i "s/\$DATABASE_PASSWORD/$DATABASE_PASSWORD/"
done
