...
cd automate/configs
./d_update-configs-from-templates.sh
cd ../..

if [ "$1" = "DOWNLOAD" ]; then
    GET=1
    IPS=`./info/get-active-node-ips.sh ALL | head -n1`
    CFGS="geograph-torquebox.yml farm-torquebox.yml infinispan-udp-conf-geograph.xml standalone.conf geograph-production.rb farm-production.rb geograph-fenix-framework-ispn.properties farm-fenix-framework-ispn.properties geograph_options.yml geograph-jgroups-tcp.xml geograph-jgroups-udp.xml jgroups-lard.xml standalone-ha.xml"
